...
/**
 * 1. Extract text from Unstructured API response
 * 2. Clean / normalize text
 * 3. Chunk with overlap + sentence awareness
 */
 ...
 */
export function extractText(elements: any): string {
  return elements
    .map((el: { text: any }) => el.text)
    .filter(Boolean)
    .join("\n");
}
